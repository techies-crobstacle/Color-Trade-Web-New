...
export default function Min3Game1() {
  ...
  const getStoredShownPeriods = (): Set<string> => {
    try {
      const stored = localStorage.getItem('shownPeriods_1m');
  ...
  const saveShownPeriods = (periods: Set<string>) => {
    try {
      localStorage.setItem('shownPeriods_1m', JSON.stringify(Array.from(periods)));
    } catch {
  ...
  const getStoredBetPeriods = (): Set<string> => {
    try {
      const stored = localStorage.getItem('userBetPeriods_1m');
      return stored ? new Set<string>(JSON.parse(stored)) : new Set<string>();
    } catch {
      return new Set<string>();
    }
  ...
  const saveBetPeriods = (periods: Set<string>) => {
    try {
      localStorage.setItem('userBetPeriods_1m', JSON.stringify(Array.from(periods)));
    } catch {
      // Silently ignore localStorage errors
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setCurrentPeriod((prev) => {
        if (prev !== newPeriod) {
          // New period started — close any open popup immediately
  ...
  // tryShowPopup: only fires once per period (guarded by resultShownPeriodsRef Set).
  // Only show popups for 1-minute game periods and only if period hasn't expired
  const tryShowPopup = (period: string, forceShow: boolean = false) => {
    if (resultShownPeriodsRef.current.has(period)) return;
    
    // Only show popups for 1m game periods
    if (!period.startsWith('1m-')) return;
    
    // Don't show popups for periods that are too old (more than 5 minutes old)
    const periodTime = parsePeriodId(period);
    if (periodTime && Date.now() - periodTime > 5 * 60 * 1000) return;

    const userEntry = userBetResultsRef.current[period];

    // Only show if user placed a bet this session
    const userPlacedBet = userBetPeriodsRef.current.has(period);
    if (!userPlacedBet) return;

    // Get winning data from available sources
    const last1m = lastGamesRef.current["1m_game"];
    const hasUserWinData = userEntry?.winningNumber != null;
    const hasLastGamesData = last1m?.period === period && last1m?.winningNumber != null;

    if (!hasUserWinData && !hasLastGamesData) return;

    const winNum = hasUserWinData ? userEntry!.winningNumber! : last1m!.winningNumber!;
    const winColors = hasUserWinData ? (userEntry!.winningColor ?? []) : (last1m!.winningColor ?? []);
    const winSize = hasUserWinData ? (userEntry!.winningSize ?? null) : (last1m!.winningSize ?? null);
    ...
    try {
      ...
      if (last1m?.winningNumber != null && last1m.period) {
        const realtimeItem: HistoryItem = {
          ...
          result: { number: last1m.winningNumber, color: last1m.winningColor || [], size: last1m.winningSize || null },
        };
        ...
        if (!mountDoneRef.current) {
          ...
          resultShownPeriodsRef.current.add(realtimeItem.period);
          saveShownPeriods(resultShownPeriodsRef.current);
        }
      } else {
        ...
        if (last4.length > 0) {
          ...
          if (!mountDoneRef.current) {
            mountDoneRef.current = true;
            resultShownPeriodsRef.current.add(last4[0].period);
            saveShownPeriods(resultShownPeriodsRef.current);
          }
        }
      }
  ...
  // ─── Sync lastGames ref + update history balls (no popup trigger) ─────────
  useEffect(() => {
    lastGamesRef.current = lastGames;
    winningHistoryRef.current = winningHistory;

    const last1m = lastGames["1m_game"];
    if (!last1m?.period || last1m.winningNumber == null || typeof last1m.winningNumber !== "number") return;

    if (last1m.period !== lastSeenPeriodRef.current) {
    ...
    // Removed: tryShowPopup(last1m.period) to prevent duplicate popups
  ...
  // Called when SocketContext's prefetch-retry completes with full result data.
  useEffect(() => {
    ...
    function updateTimer() {
      ...
      if (remaining <= 10 && remaining > 0 && game) {
        ...
        if (
          ...
          const info = lastGamesRef.current["1m_game"];
          if (
  ...
  return (
    <>
      <div className="relative">
        ...
        <div className="flex justify-between items-center px-2 2xs:px-3 py-3 2xs:py-4 bg-gradient-to-b from-[#FAE59F] to-[#C4933F] rounded-lg">
          <div className="flex flex-col gap-1 2xs:gap-2">
            <div className="flex items-center space-x-1 border border-[#8f5206] rounded-3xl px-2 2xs:px-3 py-1">
              ...
              <button className="font-light text-[#8f5206] text-xs 2xs:text-sm" onClick={() => setShowHowToPlay(true)}>How to play</button>
            </div>
            ...
            <div className="flex items-center space-x-0.5 2xs:space-x-1">
              {(() => {
                ...
                return timeString.split("").map((c, i) => (
                  <div key={i} className={clsx("flex items-center justify-center h-6 w-5 2xs:h-8 2xs:w-7 text-sm 2xs:text-base font-semibold text-white", c === ":" ? "bg-transparent text-black text-lg 2xs:text-xl" : "bg-[#333332] text-white")}>{c}</div>
                ));
              })()}
            </div>
          </div>
        ...
        <div className="flex my-6 px-3">
          {(["Big", "Small"] as const).map((bs) => (
            <button key={bs} onClick={() => handleSelect("size", bs)} disabled={disabled}
              className={clsx("w-1/2 py-3 text-white transition", bs === "Big" ? "bg-[#dd9138] rounded-l-3xl" : "bg-blue-500 rounded-r-3xl", selected.type === "size" && selected.value === bs && "ring-4 ring-yellow-400 scale-105", disabled && "opacity-60 cursor-not-allowed")}>
              {bs}
            </button>
          ))}
        </div>
        ...
        <PlaceBetModel onConfirm={placeBet} isOpen={isModalOpen} onClose={handleModalCancel} selectedBet={selected} gamePeriod={period} disabled={disabled} />
      </div>
      ...
      {showHowToPlay && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 px-4">
          <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl" style={{ backgroundColor: "#1c1c1e" }}>
            <div className="bg-gradient-to-b from-[#FAE59F] to-[#C4933F] py-3 px-4 text-center">
              <h2 className="text-white font-bold text-base tracking-wide">· Win Go 1 Min ·</h2>
            </div>
            <div className="px-4 py-4 max-h-[65vh] overflow-y-auto text-gray-200 text-sm leading-relaxed space-y-3">
              <p>1 minute 1 period, 50 seconds to order, 10 seconds waiting for the draw. It opens all day, the total number of purchases in a day is 1440 times.</p>
              <p>All single bets will have a 2% handling fee.<br />For example, if you bet 100 Rs, after deducting the fee, the exact betting amount will be 98 Rs.</p>
              <p>
                <span className="text-white font-extrabold">📌 Odds:</span>
              </p>
              <p>1. Select Green: if the result shows 1,3,7,9 the pay out is (98×2)=196; If the result shows 5, the pay out is (98×1.5)=147</p>
              <p>2. Select Red: if the result shows 2,4,6,8 the pay out is (98×2)=196; If the result shows 0, the pay out is (98×1.5)=147</p>
              <p>3. Select Violet: if the result shows 0 or 5, the pay out is (98×4.5)=441</p>
              <p>4. Select Number: if the result is the same as the number you selected, the pay out is (98×9)=882</p>
              <p>5. Select Big: if the result shows 5,6,7,8,9 the pay out is (98×2)=196</p>
              <p>6. Select Small: if the result shows 0,1,2,3,4 the pay out is (98×2)=196</p>
              <p>
                <span className="text-white font-extrabold">❗ Game Rules:</span>
              </p>
              <p>- It is not allowed to make 2-sided bets in 1 game period (For example: choosing green and red or big and small in the same period)</p>
              <p>- To bet on numbers: the maximum total number that can be selected is 7 in 1 period (No more)</p>
            </div>
