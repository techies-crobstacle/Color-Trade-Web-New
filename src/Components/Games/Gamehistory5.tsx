...
//   //         (item: any) => item.period.startsWith("3m-") && item.period.endsWith("-3")
//   //       );
...
//       const filteredBets = completedBets.filter(
//         (item: ApiHistoryItem) =>
//           item.period.startsWith("3m-") && item.period.endsWith("-3")
//       );
...
export default function HistoryTable() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/game/history?status=completed&page=1&limit=10&duration=5m`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      // API already filters by duration=5m and status=completed; sort by scheduledAt descending
