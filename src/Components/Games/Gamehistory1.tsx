...
//   //         (item: any) => item.period.startsWith("3m-") && item.period.endsWith("-1")
//   //       );
...
//       const mappedData: HistoryRow[] = lastThreeBets.map((item) => ({
//         period: item.period,
//         number: item.winningNumber ?? 0,
//         size: item.size
//           ? (item.size.charAt(0).toUpperCase() +
...
export default function HistoryTable() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/game/history?status=completed&page=1&limit=10&duration=1m`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      // API already filters by duration=1m and status=completed; sort by scheduledAt descending
    ...
    return (
      <div className="py-4 text-center text-gray-400">
        ...
        {process.env.NODE_ENV === "development" && debugInfo && (
          <p className="text-xs text-yellow-400 mt-1 break-all px-2">
            Debug: {debugInfo}
          </p>
        )}
        <button
