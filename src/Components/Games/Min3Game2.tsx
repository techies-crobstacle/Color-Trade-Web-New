...
function findActiveRound(games: GameData[]) {
  ...
  if (!foundRound) {
    foundRound = games.find(
      (r) => r.period.startsWith("3m-") && isOpenRound(r.status),
    );
  }
...
function getCurrentPeriodId() {
  ...
  return `3m-${year}${month}${day}-${hour}${minStr}`;
}
...
export default function Min3Game2() {
    ...
    try {
      ...
      if (last3m?.winningNumber != null && last3m.period) {
        const realtimeItem: HistoryItem = {
          ...
          result: {
            number: last3m.winningNumber,
            color: last3m.winningColor || [],
            size: last3m.winningSize || null,
          },
        };
  ...
  const getRemainingSeconds = (round: GameData | null) => {
    if (round?.scheduledAt) {
      ...
      if (!Number.isNaN(startMs)) {
        const duration = (round.gameDuration || ROUND_DURATION) * 1000;
        const deadlineMs = startMs + duration;
        const seconds = Math.ceil((deadlineMs - Date.now()) / 1000);
        return Math.max(0, Math.min(seconds, ROUND_DURATION));
      }
    }
  ...
  // Re-fetch history each time the period changes (every 3 minutes)
  useEffect(() => {
  ...
  // Prepend new result in real-time when a 3m game result arrives via socket
  useEffect(() => {
    lastGamesRef.current = lastGames;
    winningHistoryRef.current = winningHistory;
    const last3m = lastGames["3m_game"];
    if (
      last3m?.winningNumber != null &&
      last3m.period &&
      last3m.period !== lastSeenPeriodRef.current &&
      typeof last3m.winningNumber === 'number' // Ensure valid number
    ) {
  ...
  useEffect(() => {
    function updateTimer() {
      ...
      const remaining = getRemainingSeconds(currentGame);
      setTimer(remaining);
  ...
  setGame((prev) => {
    ...
    // If period changed, immediately create new round locally
    if (prev.period !== newPeriod) {
  ...
  const placeBet = async (amount: number) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        if (betMsgTimerRef.current) clearTimeout(betMsgTimerRef.current);
        setBetMsg({ text: err.message, ok: false });
        betMsgTimerRef.current = setTimeout(() => setBetMsg(null), 3000);
      } else {
        if (betMsgTimerRef.current) clearTimeout(betMsgTimerRef.current);
              ...
              {(() => {
                ...
                return timeString.split("").map((c, i) => (
                  <div
                    key={i}
                    className={clsx(
                      "flex items-center justify-center h-6 w-5 2xs:h-8 2xs:w-7 text-sm 2xs:text-base font-semibold",
                      c === ":"
