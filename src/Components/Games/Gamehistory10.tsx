...
// //   //         (item: any) => item.period.startsWith("3m-") && item.period.endsWith("-4")
// //   //       );
...
// //       const filteredBets = completedBets.filter(
// //         (item: ApiHistoryItem) =>
// //           item.period.startsWith("3m-") && item.period.endsWith("-4")
// //       );
...
//       const response = await fetch(
//         `${API_BASE}/api/game/history?status=completed&page=1&limit=10&duration=10m`,
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
//             "Content-Type": "application/json",
...
//       // API already filters by duration=10m and status=completed; sort by scheduledAt descending
...
//     const onRoundFinalized = () => fetchHistory();
