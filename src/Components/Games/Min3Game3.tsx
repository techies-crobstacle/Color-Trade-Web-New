...
function findActiveRound(games: GameData[]) {
  ...
  if (!foundRound) {
    foundRound = games.find(
      (r) => r.period.startsWith("5m-") && isOpenRound(r.status),
    );
  }
...
function getCurrentPeriodId() {
  ...
  return `5m-${year}${month}${day}-${hour}${minStr}`;
}
...
export default function Min3Game3() {
  ...
  // Keep currentPeriod in sync so we can react to period changes
  useEffect(() => {
    ...
    try {
      ...
      if (last5m?.winningNumber != null && last5m.period) {
        const realtimeItem: HistoryItem = {
          ...
          result: {
            number: last5m.winningNumber,
            color: last5m.winningColor || [],
            size: last5m.winningSize || null,
          },
        };
  ...
  // Re-fetch history each time the period changes (every 5 minutes)
  useEffect(() => {
  ...
  // Re-fetch when socket first connects
  useEffect(() => {
  ...
  // Prepend new result in real-time when a 5m game result arrives via socket
  useEffect(() => {
    lastGamesRef.current = lastGames;
    winningHistoryRef.current = winningHistory;
    const last5m = lastGames["5m_game"];
    if (
      last5m?.winningNumber != null &&
      last5m.period &&
      last5m.period !== lastSeenPeriodRef.current &&
      typeof last5m.winningNumber === 'number' // Ensure valid number
    ) {
  ...
  // Show popup when the server finalizes the round — lastGames["5m_game"] gets
  // updated with the current period's result (5 s before round end on the server).
  // The period check prevents showing the PREVIOUS round's result.
  ...
  useEffect(() => {
    ...
    // Only prevent VERY old popups (more than 25 minutes old)
    ...
    if (entryTime) {
      try {
        ...
        // Only block popups older than 25 minutes (to allow admin processing time)
  ...
  const placeBet = async (amount: number) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (betMsgTimerRef.current) clearTimeout(betMsgTimerRef.current);
        setBetMsg({ text: error.message || "Bet placement failed", ok: false });
        betMsgTimerRef.current = setTimeout(() => setBetMsg(null), 3000);
      } else {
        if (betMsgTimerRef.current) clearTimeout(betMsgTimerRef.current);
        setBetMsg({ text: "An unexpected error occurred", ok: false });
        betMsgTimerRef.current = setTimeout(() => setBetMsg(null), 3000);
      }
    }
  };
      ...
      <div className="relative">
        ...
        <div className="flex justify-between items-center px-3 py-4 bg-gradient-to-b from-[#FAE59F] to-[#C4933F] rounded-lg">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-[#8f5206] font-light ml-1">Win Go 5 Min</h1>

            {/* Winning number Images Last Four */}
            <div className="flex gap-1 ml-1">
              {winningHistory.length > 0 ? (
                winningHistory.map((g, idx) => (
                  <div key={idx} className="md:h-8 md:w-8 w-7 h-7 rounded-full overflow-hidden border border-white">
                    <Image
                ...
                return timeString.split("").map((c, i) => (
                  <div
                    ...
                    className={clsx(
                      ...
                      c === ":" ? "bg-transparent text-black text-xl" : "bg-[#333332] text-white"
                    )}
                  >
                    {c}
                  </div>
            ...
            <button
              ...
              className={clsx(
                ...
                c === "Green"
                  ...
                  ? "bg-purple-500"
                  : "bg-red-500",
                selected.type === "color" && selected.value === c && "ring-4 ring-yellow-400 scale-105",
          ...
          {sizeOptions.map((bs) => (
            <button
              ...
              className={clsx(
                ...
                selected.type === "size" && selected.value === bs && "ring-4 ring-yellow-400 scale-105",
                disabled && "opacity-60 cursor-not-allowed"
              )}
            >
              {bs}
