...
import GameHistory5 from "@/Components/Games/Gamehistory5";
...
function getCurrentPeriodId() {
  ...
  const minutes = Math.floor(now.getMinutes() / 5) * 5;
  return `5m-${year}${month}${day}-${hour}${pad2(minutes)}`;
}
...
export default function Min3Game3() {
  ...
  const getStoredShownPeriods = (): Set<string> => {
    try {
      const stored = localStorage.getItem('shownPeriods_5m');
  ...
  const saveShownPeriods = (periods: Set<string>) => {
    try {
      localStorage.setItem('shownPeriods_5m', JSON.stringify(Array.from(periods)));
    } catch {
  ...
  // Only show popups for 5-minute game periods and only if period hasn't expired
  const tryShowPopup = (period: string, forceShow: boolean = false) => {
    if (resultShownPeriodsRef.current.has(period)) return;
    
    // Only show popups for 5m game periods
    if (!period.startsWith('5m-')) return;
    
    // Don't show popups for periods that are too old (more than 15 minutes old)
    const periodTime = parsePeriodId(period);
    if (periodTime && Date.now() - periodTime > 15 * 60 * 1000) return;
    ...
    const last5m = lastGamesRef.current["5m_game"];
    const hasUserWinData = userEntry?.winningNumber != null;
    // Exact period match — never shows wrong period's ball
    const hasLastGamesData = last5m?.period === period && last5m?.winningNumber != null;

    if (!hasUserWinData && !hasLastGamesData) return;

    const winNum = hasUserWinData ? userEntry!.winningNumber! : last5m!.winningNumber!;
    const winColors = hasUserWinData ? (userEntry!.winningColor ?? []) : (last5m!.winningColor ?? []);
    const winSize = hasUserWinData ? (userEntry!.winningSize ?? null) : (last5m!.winningSize ?? null);
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/game/history?status=completed&page=1&limit=10&duration=5m`,
      ...
      const last5m = lastGames["5m_game"];
      if (last5m?.winningNumber != null && last5m.period) {
        const realtimeItem: HistoryItem = {
          period: last5m.period,
          scheduledAt: last5m.scheduledAt || new Date().toISOString(),
          result: { number: last5m.winningNumber, color: last5m.winningColor || [], size: last5m.winningSize || null },
        };
  ...
  const getRemainingSeconds = (_round: GameData | null) => {
    const now = new Date();
    const minutes = Math.floor(now.getMinutes() / 5) * 5;
    const periodStart = new Date(now);
    periodStart.setMinutes(minutes, 0, 0);
    return Math.max(0, ROUND_DURATION - Math.floor((now.getTime() - periodStart.getTime()) / 1000));
  };
  ...
  useEffect(() => {
    ...
    const last5m = lastGames["5m_game"];
    if (!last5m?.period || last5m.winningNumber == null || typeof last5m.winningNumber !== "number") return;

    if (last5m.period !== lastSeenPeriodRef.current) {
      lastSeenPeriodRef.current = last5m.period;
    ...
    // Removed: tryShowPopup(last5m.period) to prevent duplicate popups
    ...
    function updateTimer() {
      ...
      if (remaining <= 10 && remaining > 0 && game) {
        ...
        if (
          ...
          const info = lastGamesRef.current["5m_game"];
          if (
  ...
  return (
    <>
      <div className="relative">
        ...
        <div className="flex justify-between items-center px-2 2xs:px-3 py-3 2xs:py-4 bg-gradient-to-b from-[#FAE59F] to-[#C4933F] rounded-lg">
          <div className="flex flex-col gap-1 2xs:gap-2">
            <div className="flex items-center space-x-1 border border-[#8f5206] rounded-3xl px-2 2xs:px-3 py-1">
              ...
              <button className="text-xs 2xs:text-sm text-[#8f5206]" onClick={() => setShowHowToPlay(true)}>How to play</button>
            </div>
            <h1 className="text-[#8f5206] font-light ml-1 text-sm 2xs:text-base">Win Go 5 Min</h1>
            ...
            <div className="flex items-center space-x-1">
              {(() => {
                ...
                return timeString.split("").map((c, i) => (
                  <div key={i} className={clsx("flex items-center justify-center h-8 w-7 font-semibold", c === ":" ? "bg-transparent text-black text-xl" : "bg-[#333332] text-white")}>{c}</div>
                ));
              })()}
            </div>
          </div>
        ...
        <PlaceBetModel onConfirm={placeBet} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} selectedBet={selected} gamePeriod={game?.period} disabled={disabled} />
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 px-4">
          <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl" style={{ backgroundColor: "#1c1c1e" }}>
            <div className="bg-gradient-to-b from-[#FAE59F] to-[#C4933F] py-3 px-4 text-center">
              <h2 className="text-white font-bold text-base tracking-wide">· Win Go 5 Min ·</h2>
            </div>
